...
class TestSessionContext:
    ...
    @pytest.mark.asyncio
    async def test_store_and_retrieve_artifact(self):
        """Test storing and retrieving artifacts"""
        session = SessionContext(session_id="test_123")

        # Store artifact
        await session.store_artifact("last_diff", "diff content here", ttl=60)

        # Retrieve artifact
        value = await session.get_artifact("last_diff")
        assert value == "diff content here"

    @pytest.mark.asyncio
    async def test_artifact_expiration(self):
        """Test artifact TTL expiration"""
        session = SessionContext(session_id="test_123")

        # Store with 1 second TTL
        await session.store_artifact("temp", "expires soon", ttl=1)

        # Should exist immediately
        assert await session.get_artifact("temp") == "expires soon"

        # Wait for expiration
        time.sleep(1.1)

        # Should be None after expiration
        assert await session.get_artifact("temp") is None

    @pytest.mark.asyncio
    async def test_nonexistent_artifact(self):
        """Test retrieving nonexistent artifact"""
        session = SessionContext(session_id="test_123")

        assert await session.get_artifact("doesnt_exist") is None
    ...
    async def test_history_tracking(self):
        ...
        await session.add_to_history({"operation": "create_pr", "result": "success"})
        await session.add_to_history({"operation": "commit", "result": "success"})
...
class TestSessionManager:
    ...
    async def test_retrieve_existing_session(self):
        ...
        await session1.store_artifact("data", "test value")

        # Retrieve same session
        session2 = manager.get_or_create_session(session_id="test_session")

        assert session1 is session2
        assert await session2.get_artifact("data") == "test value"
    ...
    async def test_get_session(self):
        ...
        assert await manager.get_session("doesnt_exist") is None
    ...
    async def test_delete_session(self):
        ...
        manager.get_or_create_session(session_id="test_session")
...
async def test_concurrent_session_access():
    ...
    async def store_artifact(key, value):
        await session.store_artifact(key, value)
    ...
    assert await session.get_artifact("key1") == "value1"
    assert await session.get_artifact("key2") == "value2"
    assert await session.get_artifact("key3") == "value3"
